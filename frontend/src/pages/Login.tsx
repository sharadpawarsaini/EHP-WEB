...
const Login = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError(err.response?.data?.message || 'Invalid email or password. Please check and try again.');
    }
  };
    ...
    <div className="min-h-screen relative flex items-center justify-center py-12 px-4 sm:px-6 lg:px-8 overflow-hidden bg-gradient-to-br from-slate-50 via-blue-50/50 to-sky-100/60 dark:from-slate-950 dark:via-slate-900 dark:to-blue-950 transition-colors duration-500">
      ...
      <AnimatePresence>
        {isGoogleLoading && (
          ...
          >
            ...
            <p className="mt-6 text-xs font-bold text-blue-600 dark:text-blue-400 uppercase tracking-widest">Logging in with Google...</p>
          </motion.div>
        )}
      </AnimatePresence>
      ...
      <div className="max-w-md w-full relative z-10">
        ...
        >
          ...
          <p className="text-sm font-medium text-slate-500 dark:text-slate-400">Log in to access your Emergency Health Passport</p>
        </motion.div>
        ...
        <motion.div 
          ...
          className="bg-white/90 dark:bg-slate-900/90 backdrop-blur-2xl border border-blue-100 dark:border-slate-800 p-8 sm:p-10 rounded-[2.5rem] shadow-2xl relative overflow-hidden"
        >
        ...
        >
          <p className="text-xs font-semibold text-slate-500">
            ...
            <Link to="/register" className="text-blue-600 hover:text-blue-700 font-bold underline ml-1">Sign Up</Link>
          </p>
          <div className="mt-8 pt-6 border-t border-slate-200 dark:border-slate-800">
            <Link to="/admin/login" className="text-xs font-bold text-slate-400 hover:text-blue-600 transition-colors uppercase tracking-widest">Admin Login</Link>
          </div>
        </motion.div>
      </div>
    </div>
