      ...
      <AnimatePresence>
        {isGoogleLoading && (
          ...
          >
            ...
            <p className="mt-8 text-[10px] font-black text-emerald-500 uppercase tracking-[0.4em]">Constructing Clinical Identity...</p>
          </motion.div>
        )}
      </AnimatePresence>
      ...
      <div className="max-w-3xl w-full relative z-10">
        ...
        >
          <div className="inline-flex items-center justify-center w-20 h-20 bg-white/5 backdrop-blur-xl border border-white/10 rounded-[2rem] shadow-2xl mb-8 mx-auto group hover:scale-110 transition-transform">
            <UserPlus className="h-10 w-10 text-emerald-500 group-hover:animate-pulse" />
          </div>
          <h2 className="text-5xl font-black text-white uppercase tracking-tighter leading-none mb-4">Node Registration</h2>
          <p className="text-[10px] font-black text-zinc-500 uppercase tracking-[0.4em] italic">Initialize a new secure clinical identity node</p>
        </motion.div>
          ...
          <form className="space-y-10 relative z-10" onSubmit={handleSubmit}>
            ...
            <div className="space-y-8">
              <div className="space-y-3">
                <label className="text-[10px] font-black text-zinc-500 uppercase tracking-[0.4em] ml-2">Legal Identity</label>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-8">
                <div className="space-y-3">
                  <label className="text-[10px] font-black text-zinc-500 uppercase tracking-[0.4em] ml-2">Origin Date</label>
                  <input
                    type="date"
                    required
                    value={dob}
                    onChange={(e) => setDob(e.target.value)}
                    className="w-full px-8 py-6 bg-white/5 border border-white/10 rounded-2xl focus:ring-4 focus:ring-emerald-500/10 focus:border-emerald-500/50 outline-none text-white font-black uppercase tracking-widest text-[11px] transition-all shadow-inner"
                  />
                </div>
                <div className="space-y-3">
                  <label className="text-[10px] font-black text-zinc-500 uppercase tracking-[0.4em] ml-2">Biological Gender</label>
                  <select
                    value={gender}
                    onChange={(e) => setGender(e.target.value)}
                    className="w-full px-8 py-6 bg-zinc-950 border border-white/10 rounded-2xl focus:ring-4 focus:ring-emerald-500/10 focus:border-emerald-500/50 outline-none text-white font-black uppercase tracking-widest text-[11px] transition-all shadow-inner appearance-none"
                  >
                    <option value="Male">Protocol: Male</option>
                    <option value="Female">Protocol: Female</option>
                    <option value="Other">Protocol: Other</option>
                    <option value="Prefer not to say">Protocol: Undisclosed</option>
                  </select>
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-8">
                <div className="space-y-3">
                  <label className="text-[10px] font-black text-zinc-500 uppercase tracking-[0.4em] ml-2">Hematologic Group</label>
                  <select
                    required
                    value={bloodGroup}
                  ...
                  >
                    ...
                    {['A+', 'A-', 'B+', 'B-', 'AB+', 'AB-', 'O+', 'O-'].map(g => (
                      <option key={g} value={g}>Type: {g}</option>
                    ))}
                  </select>
                </div>
                <div className="space-y-3">
              ...
              <div className="space-y-3">
                <label className="text-[10px] font-black text-zinc-500 uppercase tracking-[0.4em] ml-2">Master Access Key</label>
                <div className="relative group/input">
                  <input
                    type="password"
                    required
            ...
            >
              Initialize Enrollment <ChevronRight className="h-5 w-5" />
            </button>
        ...
        >
          <p className="text-[10px] font-black text-zinc-500 uppercase tracking-[0.3em]">
            Existing Node? {' '}
            <Link to="/login" className="text-emerald-500 hover:text-emerald-400 transition-colors ml-2 underline underline-offset-8">Synchronize Session</Link>
          </p>
        </motion.div>
      </div>
    </div>
